...
def translate(word):
    res = u"<div style=\"font-family:微软雅黑\"><center><p><span style=\"font-size:25px; color:#f00;\">" + str(word) + "</span>"
    try:
    ...
    if jinshan!=None:
        ...
        if len(jin_word)>0:
            res+=u"<p style=\"color:#2555B4\">金山词霸</p><ul>"
    ...
    if jinshan!=None:
        ...
        if len(sents)>0: 
            res+=u"<p style=\"color:#2555B4\">习语</p><ul>"
            for sent in sents:
